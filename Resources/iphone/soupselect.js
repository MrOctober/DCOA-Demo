...
exports.select = function(dom, selector) {
    ...
    for (var i = 0; i < tokens.length; i++) {
        ...
        if (match) {
            ...
            found = [];
            for (var m = 0; m < currentContext.length; m++) "undefined" != typeof currentContext[m].children ? found = found.concat(domUtils.getElementsByTagName(tokens[i], currentContext[m].children)) : 0 === i && (found = found.concat(domUtils.getElementsByTagName(tokens[i], currentContext[m])));
            currentContext = found;
        }
    }
