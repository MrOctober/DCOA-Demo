...
//Rare exceptions to indicate a bug in the _dcoa code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_dcoa = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_dcoa]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.dcoa.module"
